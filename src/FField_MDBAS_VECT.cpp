...
FField_MDBAS_VECT::FField_MDBAS_VECT(AtomList& _at_List, PerConditions& _pbc, Ensemble& _ens,
...
FField_MDBAS_VECT::~FField_MDBAS_VECT()
{
  
}

double FField_MDBAS_VECT::getE()
{
  double ener=0.0;
...
double FField_MDBAS_VECT::getEswitch()
{
  // electrostatic and vdw are performed together for minimising computations
...
void FField_MDBAS_VECT::computeNonBonded_full()
{
...
void FField_MDBAS_VECT::computeNonBonded14()
{
...
void FField_MDBAS_VECT::computeNonBonded_switch()
{
...
void FField_MDBAS_VECT::computeNonBonded14_switch()
{
  int i, j, k;
  double lelec = 0., pelec;
  double levdw = 0., pvdw;
...
double FField_MDBAS_VECT::computeEelec(const double qi, const double qj, const double rt)
{
  return qi * qj * rt;
}

double FField_MDBAS_VECT::computeEvdw(const double epsi, const double epsj, const double sigi,
                                 const double sigj, const double rt)
{
  return epsi * epsj * (Tools::X12<double>((sigi + sigj) * rt) - Tools::X6<double>((sigi + sigj) * rt));
}

void FField_MDBAS_VECT::computeEbond()
{
  int i, j, ll;
  int type;
  double di[3], dj[3];
...
void FField_MDBAS_VECT::computeEang()
{
  int i, j, k, ll;
...
void FField_MDBAS_VECT::computeEub()
{
  int i, j, ll;
  double di[3], dj[3];
  double r0, k;
...
void FField_MDBAS_VECT::computeEdihe()
{
  int i, j, k, l, ll;
...
void FField_MDBAS_VECT::computeEimpr()
{
  int i, j, k, l, ll;
  double di[3], dj[3], dk[3], dl[3];
  double dab[3], dbc[3], dcd[3];
...
void FField_MDBAS_VECT::askListUpdate(int st)
{
  if (cutMode != FULL)
  {
