...
static void const_SELE(PARAM *param,BOND **bond,CONSTRAINT **constList);

static void write_FORF(PARAM *param,ATOM atom[],BOND bond[],ANGLE angle[],
                DIHE dihe[],DIHE impr[],CONSTRAINT constList[],
                BOND ub[],VDW vdw[]);

static void read_PSF(INPUTS *inp,PARAM *param,ATOM **atom,BOND **bond,
              ANGLE **angle,DIHE **dihe,DIHE **impr,CONSTRAINT **constList);

static void read_TOP(INPUTS *inp);

static void read_PAR(INPUTS *inp);

static void read_CONF(PARAM *param,ATOM atom[]);

static void setup(INPUTS *inp,PARAM *param,ATOM atom[],BOND bond[],
          ANGLE angle[],DIHE dihe[],DIHE impr[],CONSTRAINT constList[],
          BOND **ub,VDW **vdw);

static void free_temp_array(INPUTS *inp);
...
//output file
static const char forfname[]="ffield.dat";

static char corname[256];
static char psfname[256];
static char topolname[256];
static char parname[256];
...
int main(int argc, char** argv)
{
    ...
    for (i=1; i<argc; i++)
    {
      // get name of cor file
      if (!strcasecmp(argv[i],"-cor"))
      {
        sprintf(corname,"%s",argv[++i]);
      }
      // psf file
      else if(!strcasecmp(argv[i],"-psf"))
      {
        sprintf(psfname,"%s",argv[++i]);
      }
      // top file
      else if(!strcasecmp(argv[i],"-top"))
      {
        sprintf(topolname,"%s",argv[++i]);
      }
      // par file
      else if(!strcasecmp(argv[i],"-par"))
      {
        sprintf(parname,"%s",argv[++i]);
      }
    ...
    printf("MDBAS compatible forcefield generated and saved to file : %s\n",forfname);
...
static void write_FORF(PARAM *param,ATOM atom[],BOND bond[],ANGLE angle[],
                ...
                BOND ub[],VDW vdw[])
{
    FILE *forfFile=fopen(forfname,"w");
    int i,j,nDihe,nImpr;

    fprintf(forfFile,"# Force Field file for MDBas compatible codes\n");
...
static void error(int errorNumber)
{
    printf("%s failed due to error number: %d\n",progname,errorNumber);
    switch (errorNumber)
    {
    case 10:
        printf("%s cannot find or open topology file TOP.\n",progname);
        printf("Most likely, it is not properly named. Please check.\n");
        break;
    case 20:
      printf("%s cannot find or open structure file PSF.\n",progname);
        printf("Most likely, it is not properly named. Please check.\n");
        break;
    case 22:
      printf("%s encountered a problem while reading atomic properties\n",progname);
    ...
    case 26:
        ...
        break;
    case 30:
      printf("%s cannot find or open parameter file PAR.\n",progname);
        printf("Most likely, it is not properly named. Please check.\n");
        break;
    case 40:
      printf("%s cannot find or open configuration file CONF.\n",progname);
        printf("Most likely, it is not properly named. Please check.\n");
        break;
    case 41:
      printf("%s found a different number of atoms in CONF file and\n",progname);
        printf("in PSF file. Structure does not match configuration.\n");
        printf("Check carefully these files.\n");
        break;
    case 47:
      printf("%s cannot open configuration file RESCONF.\n",progname);
        break;
    case 50:
        ...
        break;
    case 60:
      printf("%s cannot find or open simulation file SIMU.\n",progname);
        printf("Most likely, it is not properly named. Please check.\n");
        break;
    case 61:
      printf("%s does not recognise a keyword specified in SIMU.\n",progname);
        printf("Please check SIMU file and the manual for the list of\n");
        printf("allowed keywords.\n");
        break;
    case 62:
      printf("%s does not recognise a parameter specified in SIMU.\n",progname);
        printf("Please check SIMU file and the manual for the list of\n");
        printf("allowed keywords and their associated parameters.\n");
        break;
    case 63:
      printf("%s does not find a required parameter in SIMU.\n",progname);
        printf("Please check SIMU file and the manual for the list of\n");
        printf("allowed keywords and their associated parameters.\n");
        break;
    case 71:
    ...
    case 74:
        ...
        break;
    case 110:
      printf("%s found a too many non-parameterised dihedral angles:\n",progname);
        ...
        printf("the size you really need. Then recompile %s.\n",progname);
        break;
    case 111:
      printf("%s encountered a problem while setting the excluded atoms\n",progname);
        ...
        printf("by %s. Sorry for the trouble.\n",progname);
        break;
    case 201:
    ...
    case 202:
        ...
        break;
    default:
      printf("%s failed due to unknown error number: %d\n",progname,errorNumber);
